              ...
              >
                ...
                >
                  <div className="min-w-0 pr-3">
                    <div className="flex flex-wrap items-center gap-2">
                      ...
                      {/* Theme Song Badge */}
                      {song.isThemeSong && (
                        <span className="px-2 py-0.5 rounded-md text-[10px] font-bold bg-violet-50 dark:bg-violet-950/60 text-violet-700 dark:text-violet-300 border border-violet-200 dark:border-violet-800">
                          Theme Song
                        </span>
                      )}
                      ...
                      {history.isRecent && (
                        <span
                          className="inline-flex items-center gap-1 px-1.5 py-0.5 rounded text-[10px] font-medium bg-rose-50 dark:bg-rose-950/50 text-rose-700 dark:text-rose-400 border border-rose-200/60 dark:border-rose-800/60 shrink-0"
                    ...
                    <button
                      ...
                      className={`p-2 rounded-xl transition-all cursor-pointer select-none ${
                        song.isStarred || (song as any).starred
                          ? 'text-yellow-400 hover:text-yellow-500 hover:bg-yellow-50/80 dark:hover:bg-yellow-950/30'
                          : 'text-slate-300 dark:text-slate-600 hover:text-yellow-400 dark:hover:text-yellow-400 hover:bg-slate-100 dark:hover:bg-slate-800'
                      }`}
                      ...
                      <Star
                        className={`w-5 h-5 transition-transform active:scale-125 ${
                          song.isStarred || (song as any).starred
                            ...
                            : 'text-slate-300 dark:text-slate-600 hover:text-yellow-400 stroke-[1.75]'
                        }`}
                      />
                    </button>
                  </div>
                ...
                {isSelected && (
                  <div className="px-4 sm:px-6 pb-6 pt-2 border-t border-slate-100 dark:border-slate-800 space-y-5">
                      ...
                      <div className="flex min-w-0 items-center gap-1.5 sm:gap-2">
                        {onBackToSetlist && returnSetlistId && (
                          <button
                            ...
                            className="p-2 sm:px-3 sm:py-1.5 rounded-xl bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold flex shrink-0 items-center gap-1.5 shadow-xs transition-all cursor-pointer"
                            title="Back to Set List"
                            aria-label="Back to Set List"
                          >
                            <ArrowLeft className="w-4 h-4 stroke-[2.5]" />
                            <span className="hidden sm:inline">Back to Set List</span>
                          </button>
                        )}
                        <button
                          onClick={() => {
                          ...
                          className="p-2 rounded-xl bg-slate-900 dark:bg-slate-100 text-white dark:text-slate-900 flex shrink-0 items-center justify-center shadow-xs hover:bg-slate-800 dark:hover:bg-white cursor-pointer"
                        ...
                        <div className="flex h-8 shrink-0 items-center overflow-hidden rounded-xl border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-800 shadow-2xs">
                          <button
                            type="button"
                            onClick={() => setInCardFontSize((prev) => Math.max(0, prev - 1))}
                            disabled={inCardFontSize <= 0}
                            className="h-full w-8 text-xs font-black text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-30 cursor-pointer"
                            title={`Decrease font size (currently ${['15px', '18px', '22px', '28px'][inCardFontSize]})`}
                          ...
                          <button
                            ...
                            className="h-full w-8 border-l border-slate-200 dark:border-slate-700 text-xs font-black text-slate-600 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-700 disabled:opacity-30 cursor-pointer"
                            title={`Increase font size (currently ${['15px', '18px', '22px', '28px'][inCardFontSize]})`}
                      ...
                      <div className="relative ml-auto shrink-0" onClick={(e) => e.stopPropagation()}>
                        ...
                        {isMenuOpen && (
                          <div className="absolute right-0 top-full mt-1.5 w-56 rounded-2xl bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 shadow-xl py-1.5 z-40 space-y-0.5">
                            ...
                            >
                              <Star className={`w-3.5 h-3.5 ${song.isStarred || (song as any).starred ? 'fill-yellow-400 text-yellow-400' : 'text-yellow-400'}`} />
                    ...
                    {/* Song Card Footer (Attachment button) */}
                    <div
                      id={`song-footer-${song.id}`}
                      className="mt-4 pt-3 pb-1 border-t border-slate-100 dark:border-slate-800 flex items-center justify-center"
                    >
                      <button
                        type="button"
                        onClick={(e) => {
